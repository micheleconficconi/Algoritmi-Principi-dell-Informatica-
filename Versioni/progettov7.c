//differenze dalla precedente: ho check in controlla e prepara
...
int collisioni_ric=0, collisioni_mag=0;
        ...
        if(Ricettario->vett[Hk]==NULL){ //inserimento su cella vuota 
            ...
            while(c!='\n'){
                if(scanf("%s %d%c", r, &k, &c)>0){ //<ingrediente> <quantita>
                    ...
                    vett[i].nome=(char*)malloc(sizeof(char)*(vett[i].strlen+1)); //alloca una stringa con la dim giusta per contenere il nome dell'ingrediente
            ...
            for(i=0; i<n_ingredienti; i++){ //copio dal vettore grosso in quello di dim giusta
                ...
                Ricettario->vett[Hk]->vett[i].hash=Funzione_hash_M(Magazzino->dim, vett[i].nome, vett[i].strlen);
            ...
            if(Ricettario->vett[Hk]->prox==NULL){ //inserisco direttamente su quella dopo, nel bucket c'e' solo una 
                ...
                for(i=0; i<n_ingredienti; i++){ //copio dal vettore grosso in quello di dim giusta
                    ...
                    Ricettario->vett[Hk]->prox->vett[i].hash=Funzione_hash_M(Magazzino->dim, vett[i].nome, vett[i].strlen);
                ...
                if(punt==NULL){ //se c'è spazio vuoto aggiungo
                    ...
                    for(i=0; i<n_ingredienti; i++){ //copio dal vettore grosso in quello di dim giusta
                        ...
                        prev->prox->vett[i].hash=Funzione_hash_M(Magazzino->dim, vett[i].nome, vett[i].strlen);
            ...
            if(scadenza>tempo){
                ...
                if(Magazzino->vett[Hk]==NULL){ //caso 1)
                    ...
                    if(ptr->next==NULL){ //a)il nodo non c'è =>ne alloco uno nuovo
                        collisioni_mag++;
                        printf("collisione magazzino n %d\n", collisioni_mag);
...
int ControllaEPrepara(RICETTA_p Ricetta, MAGAZZINO_P Magazzino, LO_Poynter Lista_Ordini, int num, char * r, int flag){
    int i, n, peso_tolto, k=Ricetta->n_ingredienti; 
