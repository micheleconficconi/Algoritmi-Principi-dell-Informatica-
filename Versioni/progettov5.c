//differenza dalla precedente: min-heap nel magazzino al posto dei RBT
...
int ControllaEPrepara(RICETTA_p, MAGAZZINO_P, Ingrediente *, LO_Poynter, int, char *, int);
...
void Controlla_Ordini_Attesa(LO_Poynter Lista_Ordini, MAGAZZINO_P Magazzino, RICETTARIO_P Ricettario){ 
    ...
    while(ptr!=NULL){             
        ...
        peso_ordine=ControllaEPrepara(ptr->Ricetta, Magazzino, pt, Lista_Ordini, ptr->quantita, ptr->ordine, 1);
...
void Ordine(RICETTARIO_P Ricettario, MAGAZZINO_P Magazzino, LO_Poynter Lista_Ordini){
    ...
    if(scanf("%s %d", r, &num)>0){
        ...
        if((punt==NULL)){
            ...
            peso_ordine=ControllaEPrepara(punt, Magazzino, ptr, Lista_Ordini, num, r, 0);
            if(peso_ordine){
                printf("accettato\n");
...
int ControllaEPrepara(RICETTA_p Ricetta, MAGAZZINO_P Magazzino, Ingrediente * primo_ingr_ricetta, LO_Poynter Lista_Ordini, int num, char * r, int flag){
    int n, peso_tolto; 
