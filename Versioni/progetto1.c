//versione con controlla e prepara senza check
...
int ControllaEPrepara(MAGAZZINO_P Magazzino, Ingrediente * ricetta, LO_Poynter Lista_Ordini, int num, char * r, int flag){//restituisce peso tot se ha preparato ricetta, se no 0
    ...
    while(ptr!=NULL){
        ...
        Mk=Funzione_hash_M_new(Magazzino, Magazzino->dimensione, ptr->nome, strlen(ptr->nome)); //individuo l'albero che mi serve
