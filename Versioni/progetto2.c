...
void RBT_pretty_print(Lotto * root);
void RBT_print_node(Lotto * root);
void RBT_pretty_print_inner(Lotto * root, char *prefix, int isTail);
...
int main( ){
    ...
    minHeap->V=(Ordine_Stampabile *)calloc(256, sizeof(Ordine_Stampabile));
    ...
    while(c!='\n'){
        if(scanf("%s %d %d%c", r, &quantita, &scadenza, &c)>0){ //<ingrediente> <qt> <scadenza> ...
            if(scadenza>tempo){
                ...
                // RBT_pretty_print(Magazzino->vett[Hk]);
            }
...
int ControllaEPrepara(MAGAZZINO_P Magazzino, Ingrediente * primo_ingr_ricetta, LO_Poynter Lista_Ordini, int num, char * r, int flag){//restituisce peso tot se ha preparato ricetta, se no 0
    ...
    while(ptr!=NULL){ //prima controllo gli ingredienti, se li ho preparo (=>rimuovo gli ingredienti e aggiungo a LO->OP)
        ...
        if(Magazzino->vett[Mk]==T_null){ // se l'ingrediente non c'è l'ordine va in attesa
            ...
            // RBT_pretty_print(Magazzino->vett[Mk]);
...
// https://sethfurman.github.io/programming/c/2017/05/13/red-black-tree-c.html
// helper: Print a given node and its metadata.
void RBT_print_node(Lotto * root) {
    switch (root->color) {
        case 'b': printf(RBT_BOLD_BLACK); break;
        case 'r': printf(RBT_BOLD_RED); break;
        default: printf(RBT_BOLD_RED); break;
    }
    printf("%s scad:%d qty:%d", root->ingrediente, root->scadenza, root->quantita);
    printf(RBT_NO_STYLE);
    printf("\n");
}
...
void RBT_pretty_print(Lotto * root) {
    ...
    // Print any subtrees
    if (root->right == NULL) {
        RBT_pretty_print_inner(root->left, "", 1);
    } else if (root->left == NULL) {
        RBT_pretty_print_inner(root->right, "", 1);
    } else {
        RBT_pretty_print_inner(root->right, "", 0);
        RBT_pretty_print_inner(root->left, "", 1);
    }
}
...
void RBT_pretty_print_inner(Lotto * root, char *prefix, int isTail) {
    ...
    if (str != NULL) {
        if (root->right == NULL) {
            RBT_pretty_print_inner(root->left, str, 1);
        } else if (root->left == NULL) {
            RBT_pretty_print_inner(root->right, str, 1);
        } else {
            RBT_pretty_print_inner(root->right, str, 0);
            RBT_pretty_print_inner(root->left, str, 1);
        }
    } else {
