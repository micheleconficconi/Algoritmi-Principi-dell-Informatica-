//differenze dalla precedente: invece che ricalcolare ogni volta check lo salvo dinamicamente nell'ordine in attesa
...
int ControllaEPrepara(RICETTA_p, MAGAZZINO_P, LO_Poynter, Lista_Ordini_Attesa, int, char *, int);
...
void Controlla_Ordini_Attesa(LO_Poynter Lista_Ordini, MAGAZZINO_P Magazzino, RICETTARIO_P Ricettario){ 
    ...
    while(ptr!=NULL){             
        ...
        peso_ordine=ControllaEPrepara(ptr->Ricetta, Magazzino, Lista_Ordini, ptr, ptr->quantita, ptr->ordine, 1);
...
void Ordine(RICETTARIO_P Ricettario, MAGAZZINO_P Magazzino, LO_Poynter Lista_Ordini){
    ...
    if(scanf("%s %d", r, &num)>0){
        ...
        if((punt==NULL)){
            ...
            peso_ordine=ControllaEPrepara(punt, Magazzino, Lista_Ordini, NULL, num, r, 0);
            if(peso_ordine){
                printf("accettato\n");
...
int ControllaEPrepara(RICETTA_p Ricetta, MAGAZZINO_P Magazzino, LO_Poynter Lista_Ordini, Lista_Ordini_Attesa ptr, int num, char * r, int flag){
    int i, n, peso_tolto, k=Ricetta->n_ingredienti, ingrediente_n=0, p; 
    MHmagazzino punt=NULL, Check[k];
    if(flag){
        if(ptr->check==NULL){ //lo alloco nuovo
            ptr->check=(MHmagazzino *)malloc(sizeof(MHmagazzino)*k);
            ptr->check_dim=0;
        }
        p=ptr->check_dim;
        for(i=0; i<p; i++){ //controlla le quantita totali direttamente dal check dell'OA
            n=Ricetta->vett[i].quantita*num; //quantita totale per quell'ingrediente, in questo ordine 
            if(ptr->check[i]->minHeap_M->numero_elem>0){
                Controlla_Scadenze(ptr->check[i]);
                if(ptr->check[i]->qt_tot<n)
                    return 0;
            }
            else return 0;
        }
        for(i=p; i<k; i++){
            ...
            if(Magazzino->vett[Ricetta->vett[i].hash]==NULL)
                return 0;
            else{
                punt =Magazzino->vett[Ricetta->vett[i].hash];
                while((punt!=NULL)&&(ingrediente_n!=punt->ingrediente_n))
                    punt=punt->next;
                if(punt==NULL)
                    return 0;
                else{
        ...
        } //se esco da questo for ho tutte le quantità e devo rimuoverle
        for(i=0; i<Ricetta->n_ingredienti; i++){
            n=Ricetta->vett[i].quantita*num; //ottimizzazione: n lo inserisco nella struct di check fatta da MHmagazzino e da int n;
            while((ptr->check[i]->minHeap_M->numero_elem>0)&&(n>=ptr->check[i]->minHeap_M->vett[0].quantita)){ //devo inserire il controllo sul numero di elementi perche' può essere che ci siano esattamente le quantita giuste
                peso_tolto=Estrai_Min_M(ptr->check[i]->minHeap_M);
                ptr->check[i]->qt_tot-=peso_tolto;
                n-=peso_tolto; 
            }
            if(n>0){
                ptr->check[i]->minHeap_M->vett[0].quantita-=n;
                ptr->check[i]->qt_tot-=n;
            }
        }
