//differenze dalla precedente: salvo il numero degli ingredienti nella ricetta e il while in ctrleprep diventa for, 
//e alloco un vettore della dimesnione giusta per salvare gli ingredienti invece che una lista
...
#define DIM_RIC_BASE 65536
#define DIM_MAG_BASE 65536
#define HASH_CONSTANT_R ((double) (sqrt(5)-1)/2)
#define HASH_CONSTANT_M ((double) (sqrt(5)-1)/2)
#define HEAP_SIZE_O 32768
#define HEAP_SIZE_M 20;
#define DIM_MAX_VETT 51;
...
int primo_OA=1; //per resettare l'inserimento nella lista degli OA;
int primo_OS=1; //uguale a primo_OA ma per gli ordini stampabili;
int main( ){
    ...
    minHeap->dimensione=HEAP_SIZE_O;
...
void Aggiungi_Ricetta(RICETTARIO_P Ricettario, MAGAZZINO_P Magazzino){
    ...
    Ingrediente_vett vett[dim]; //serve per allocare gli ingredienti della ricetta
        ...
        if(Ricettario->vett[Hk]==NULL){ //inserimento su cella vuota 
            ...
            while(c!='\n'){
                if(scanf("%s %d%c", r, &k, &c)>0){ //<ingrediente> <quantita>
                    vett[i].quantita=k;
                    peso+=k;
                    vett[i].strlen=strlen(r); //lunghezza nome ingrediente
                    vett[i].nome=(char*)malloc(sizeof(char)*(vett[i].strlen+1)); //alloca una stringa con la dimensione giusta per contenere il nome dell'ingrediente
                    strcpy(vett[i].nome, r);  //salva il nome dell'ingrediente
                    i++;
                    n_ingredienti++;
                }
            }
            //uscito dal while alloco un vettore della dimensione giusta per tenere gli ingredienti
            Ricettario->vett[Hk]->vett=(Ingrediente *)malloc(sizeof(Ingrediente)*n_ingredienti);
            for(i=0; i<n_ingredienti; i++){ //copio dal vettore grosso in quello di dimensione giusta
                Ricettario->vett[Hk]->vett[i].quantita=vett[i].quantita;
                Ricettario->vett[Hk]->vett[i].hash=Funzione_hash_M(Magazzino, Magazzino->dimensione, vett[i].nome, vett[i].strlen);
                Ricettario->vett[Hk]->vett[i].nome=(char*)malloc(sizeof(char)*(vett[i].strlen+1));
                strcpy(Ricettario->vett[Hk]->vett[i].nome, vett[i].nome);
                free(vett[i].nome);
            }
            ...
            if(Ricettario->vett[Hk]->prox==NULL){ //inserisco direttamente su quella dopo, nel bucket c'e' solo una ricetta
                ...
                for(i=0; i<n_ingredienti; i++){ //copio dal vettore grosso in quello di dimensione giusta
                    ...
                    Ricettario->vett[Hk]->prox->vett[i].hash=Funzione_hash_M(Magazzino, Magazzino->dimensione, vett[i].nome, vett[i].strlen);
                    Ricettario->vett[Hk]->prox->vett[i].nome=(char*)malloc(sizeof(char)*(vett[i].strlen+1));
                    strcpy(Ricettario->vett[Hk]->prox->vett[i].nome, vett[i].nome);
                    free(vett[i].nome);
                }
                ...
                if(punt==NULL){ //se c'è spazio vuoto aggiungo
                    ...
                    for(i=0; i<n_ingredienti; i++){ //copio dal vettore grosso in quello di dimensione giusta
                        ...
                        prev->prox->vett[i].hash=Funzione_hash_M(Magazzino, Magazzino->dimensione, vett[i].nome, vett[i].strlen);
                        prev->prox->vett[i].nome=(char*)malloc(sizeof(char)*(vett[i].strlen+1));
                        strcpy(prev->prox->vett[i].nome, vett[i].nome);
                        free(vett[i].nome);
                    }
                    prev->prox->peso=peso;
    ...
    if(scanf("%s", r)>0){
        ...
        if(punt!=NULL){ //in questo momento se entro nell'if, punt punta alla ricetta da rimuovere
            if((Check_Attesa(Lista_Ordini->OA, r))+(Check_Ordini_Pronti(Lista_Ordini->OP, r))){ //controllo che non ci siano ordini in attesa/pronto con quella ricetta
                ...
                for(i=0; i<punt->n_ingredienti; i++){
                    free(punt->vett[i].nome);
                }
                free(punt->vett);
                free(punt->nome);
...
void Rifornimento(MAGAZZINO_P Magazzino, RICETTARIO_P Ricettario, LO_Poynter Lista_Ordini){
    char r[256], c='a';
    int quantita, scadenza, Hk, k=HEAP_SIZE_M;
    size_t lunghezza;
        ...
        if(scanf("%s %d %d%c", r, &quantita, &scadenza, &c)>0){ //<ingrediente> <qt> <scadenza> ...
            if(scadenza>tempo){
                ...
                if(Magazzino->vett[Hk]==NULL){ //caso 1)
                    ...
                    Magazzino->vett[Hk]->minHeap_M->dimensione=HEAP_SIZE_M;
                    ...
                    Magazzino->vett[Hk]->qt_tot+=quantita;
                    if(Magazzino->vett[Hk]->minHeap_M->numero_elem==Magazzino->vett[Hk]->minHeap_M->dimensione-1)
                    ...
                    if(ptr->next==NULL){ //a)il nodo non c'è =>ne alloco uno nuovo
                        ...
                        ptr->next->minHeap_M->dimensione=HEAP_SIZE_M;
                        ...
                        ptr->next->qt_tot+=quantita;
                        if(ptr->next->minHeap_M->numero_elem==ptr->next->minHeap_M->dimensione-1)
...
void Inserisci_M (minHeap_M_P minHeap, char * r, int quantita, int scadenza){
    ...
    minHeap->vett[minHeap->numero_elem-1].ingrediente=(char*)malloc(sizeof(char)*(strlen(r)+1));
    strcpy(minHeap->vett[minHeap->numero_elem-1].ingrediente, r);
    ...
    while((i>0)&&(minHeap->vett[k].scadenza>minHeap->vett[i].scadenza)){
        ...
        tmp.ingrediente=(char*)malloc(sizeof(char)*(strlen(minHeap->vett[k].ingrediente)+1));
        strcpy(tmp.ingrediente, minHeap->vett[k].ingrediente);
        //copio i dati di V[i] in V[k]
        minHeap->vett[k].quantita=minHeap->vett[i].quantita;
        minHeap->vett[k].scadenza=minHeap->vett[i].scadenza;
        free(minHeap->vett[k].ingrediente);
        minHeap->vett[k].ingrediente=(char*)malloc(sizeof(char)*(strlen(minHeap->vett[i].ingrediente)+1));
        strcpy(minHeap->vett[k].ingrediente, minHeap->vett[i].ingrediente);
        //ora metto tmp in V[i]
        minHeap->vett[i].scadenza=tmp.scadenza;
        minHeap->vett[i].quantita=tmp.quantita;
        free(minHeap->vett[i].ingrediente);
        minHeap->vett[i].ingrediente=(char*)malloc(sizeof(char)*(strlen(tmp.ingrediente)+1));
        strcpy(minHeap->vett[i].ingrediente, tmp.ingrediente);
...
void MinheapIfy_M(minHeap_M_P minHeap, int n){
    ...
    if(posmax!=n){
        ...
        tmp.ingrediente=(char*)malloc(sizeof(char)*(strlen(minHeap->vett[n].ingrediente)+1));
        strcpy(tmp.ingrediente, minHeap->vett[n].ingrediente);
        //copio i dati di V[i] in V[k]
        minHeap->vett[n].quantita=minHeap->vett[posmax].quantita;
        minHeap->vett[n].scadenza=minHeap->vett[posmax].scadenza;
        free(minHeap->vett[n].ingrediente);
        minHeap->vett[n].ingrediente=(char*)malloc(sizeof(char)*(strlen(minHeap->vett[posmax].ingrediente)+1));
        strcpy(minHeap->vett[n].ingrediente, minHeap->vett[posmax].ingrediente);
        //ora metto tmp in V[i]
        minHeap->vett[posmax].scadenza=tmp.scadenza;
        minHeap->vett[posmax].quantita=tmp.quantita;
        free(minHeap->vett[posmax].ingrediente);
        minHeap->vett[posmax].ingrediente=(char*)malloc(sizeof(char)*(strlen(tmp.ingrediente)+1));
        strcpy(minHeap->vett[posmax].ingrediente, tmp.ingrediente);
        free(tmp.ingrediente);
...
int Estrai_Min_M(minHeap_M_P minHeap){
    ...
    free(minHeap->vett[0].ingrediente);
    minHeap->vett[0].ingrediente=(char*)malloc(sizeof(char)*(strlen(minHeap->vett[minHeap->numero_elem-1].ingrediente)+1));
    strcpy(minHeap->vett[0].ingrediente, minHeap->vett[minHeap->numero_elem-1].ingrediente);
    free(minHeap->vett[minHeap->numero_elem-1].ingrediente);
    minHeap->numero_elem--;
...
void Espandi_MinHeap(minHeap_M_P minHeap){
    ...
    minHeap->dimensione=minHeap->dimensione*2; //raddoppio la dimensione
    vett_new=(Lotto*)malloc(sizeof(Lotto)*(minHeap->dimensione));
    ...
    while(i<k){
        ...
        vett_new[i].ingrediente=(char*)malloc(sizeof(char)*(strlen(minHeap->vett[i].ingrediente)+1));
        strcpy(vett_new[i].ingrediente, minHeap->vett[i].ingrediente);
        i++;
    }
...
int ControllaEPrepara(RICETTA_p Ricetta, MAGAZZINO_P Magazzino, LO_Poynter Lista_Ordini, int num, char * r, int flag){
    ...
    for(i=0; i<Ricetta->n_ingredienti; i++){
        ...
        if(Magazzino->vett[Ricetta->vett[i].hash]==NULL){ //non c'è alcuna quantita di quell' ingrediente
            ...
            while((punt!=NULL)&&(strcmp(punt->ingrediente, Ricetta->vett[i].nome)!=0)) //cerco il nodo corrispondente a quell'ingrediente
                punt=punt->next;
    ...
    for(i=0; i<Ricetta->n_ingredienti; i++){
        ...
        while((punt!=NULL)&&(strcmp(punt->ingrediente, Ricetta->vett[i].nome)!=0)) //cerco il nodo corrispondente a quell'ingrediente
            punt=punt->next;
        while((punt->minHeap_M->numero_elem>0)&&(n>=punt->minHeap_M->vett[0].quantita)){ //devo inserire il controllo sul numero di elementi perche' può essere che ci siano esattamente le quantita giuste
            peso_tolto=Estrai_Min_M(punt->minHeap_M);
            punt->qt_tot-=peso_tolto;
            n-=peso_tolto; 
        }
        if(n>0){
            punt->minHeap_M->vett[0].quantita-=n;
            punt->qt_tot-=n;
        }
    }
