...
void Aggiungi_Ricetta(RICETTARIO_P Ricettario, MAGAZZINO_P Magazzino){
    ...
    if(scanf("%s", r)>0){ //copio il nome della ricetta
        ...
        if(Ricettario->vett[Hk]==NULL){ //inserimento su cella vuota 
            ...
            Ricettario->vett[Hk]->prox=NULL;
            printf("aggiunta\n");
            return;
        } 
        else if(strcmp(Ricettario->vett[Hk]->nome, r)!=0){ //caso in cui è già presente una ricetta ma con nome diverso
            if(Ricettario->vett[Hk]->prox==NULL){ //inserisco direttamente su quella dopo
            ...
            else{ //caso in cui la seconda è !=NULL
...
void Rimuovi_Ricetta(RICETTARIO_P Ricettario, LO_Poynter Lista_Ordini){
    char r[256];
    int Hk;             //potrei dichiarare lunghezza come int e castare il risultato di strlen a int
    size_t lunghezza;
    ...
    if(scanf("%s", r)>0){
        ...
        if(punt!=NULL){ //in questo momento se entro nell'if, punt punta alla ricetta da rimuovere
            if((Check_Attesa(Lista_Ordini->OA, r))+(Check_Ordini_Pronti(Lista_Ordini->OP, r))){ //controllo che non ci siano ordini in attesa con quella ricetta
...
void Controlla_Ordini_Attesa(LO_Poynter Lista_Ordini, MAGAZZINO_P Magazzino, RICETTARIO_P Ricettario){ //deve scorrere gli ordini in attesa e vedere se sono preparabili o no 
    ...
    while(ptr!=NULL){                                                          //ricetta per non dover ricarlcolare la funzione di hash
        ...
        punt=Ricettario->vett[Hk];
        while((punt!=NULL)&&(strcmp(punt->nome, ptr->ordine)!=0))
            punt=punt->prox;
        pt=punt->next; //pt punta al primo ingrediente
        peso_ordine=ControllaEPrepara(punt, Magazzino, pt, Lista_Ordini, ptr->quantita, ptr->ordine, 1);
