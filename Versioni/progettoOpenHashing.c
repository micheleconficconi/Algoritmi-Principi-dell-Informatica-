...
CHECK Enqueue_CK (CHECK, Lotto *, int, int);
...
int ControllaEPrepara(MAGAZZINO_P Magazzino, Ingrediente * primo_ingr_ricetta, LO_Poynter Lista_Ordini, int num, char * r, int flag){//restituisce peso tot se ha preparato ricetta, se no 0
    ...
    Lotto * pt=NULL;
    //  *tmp=NULL; //ptr scorre la ricetta puntando all'inizio al primo ingrediente, pt scorre gli alberi
    while(ptr!=NULL){ //prima controllo gli ingredienti, se li ho preparo (=>rimuovo gli ingredienti e aggiungo a LO->OP)
        ...
        if(Magazzino->vett[Mk]==T_null){ // se l'ingrediente non c'è l'ordine va in attesa
            ...
            if(Magazzino->vett[Mk]!=T_null){ //se l'albero non è stato toalmente eliminato da controlla_scadenze
                ...
                Check=Enqueue_CK(Check, pt, Mk, residuo);
    ...
    //fintanto che ho elem da spilare, spilo
    while (Check!=NULL)
    {
        Check=Deueue_CK(Check, Magazzino);
    }
    
    return peso_tot;
}

// CHECK Enqueue_CK (CHECK coda, CHECK testa, Lotto * ptr, int Mk, int residuo){
//     CHECK punt, cur=coda;
...
CHECK Enqueue_CK (CHECK coda, Lotto * ptr, int Mk, int residuo){
    CHECK punt;
    punt=(CHECK)malloc(sizeof(Nodo));
    punt->ingrediente=ptr;
    punt->Hash=Mk;
...
//spila precomputata lista in LIFO e aggiorna residui di magazzino
CHECK Deueue_CK (CHECK coda, MAGAZZINO_P Magazzino){
    CHECK tmp;
    if (coda->dacanc > 0){
        RB_Delete_M(Magazzino, coda->Hash, coda->ingrediente);
    }
    else {
        coda->ingrediente->quantita=-(coda->dacanc);
    }
