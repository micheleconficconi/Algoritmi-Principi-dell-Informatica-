...
typedef RBTm * RBTmagazzino;
 ...
 //questi puntatori, cioè le teste degli alberi, sono gli elementi del magazzino
...
int ControllaEPrepara(RICETTARIO_P, int, MAGAZZINO_P, Ingrediente *, LO_Poynter, int, char *, int);
...
CHECK Enqueue_CK (CHECK, RBTmagazzino);
void free_Check(CHECK);
CHECK Deueue_CK (CHECK, MAGAZZINO_P);
...
void Aggiungi_Ricetta(RICETTARIO_P Ricettario, MAGAZZINO_P Magazzino){
    char r[256], c='a';
    int k=0, Hk=0, n_ingredienti=0, peso=0;
    size_t lunghezza;
...
int Funzione_hash_R_new(RICETTARIO_P Ricettario, int m, char * r, size_t lunghezza, int p){  
    ...
    i=0;
    if(p){ //ricerca: se trovi tomb vai avanti
    ...
    if(scanf("%s", r)>0){
        ...
        if(Ricettario->vett[Hk].nome!=NULL){
            ...
            if((compare==0)&&((Check_Attesa(Lista_Ordini->OA, r))+(Check_Ordini_Pronti(Lista_Ordini->OP, r)))){
                ...
                Ricettario->vett[Hk].n_ingredienti=0;
                Ricettario->vett[Hk].peso=0;
                printf("rimossa\n");
                return;
            }
            ...
            if(scadenza>tempo){
                ...
                if(Magazzino->vett[Hk]==NULL){ //caso 1)
                    ...
                    while((ptr->next!=NULL)&&(strcmp(ptr->next->ingrediente, r)!=0))
                        ptr=ptr->next;
                    if(ptr->next==NULL){ //a)il nodo non c'è
...
void Controlla_Scadenze(RBTmagazzino Root, Lotto * ptr){
...
void Controlla_Ordini_Attesa(LO_Poynter Lista_Ordini, MAGAZZINO_P Magazzino, RICETTARIO_P Ricettario){ //deve scorrere gli ordini in attesa e vedere se sono preparabili o no 
    ...
    while(ptr!=NULL){                                                          //ricetta per non dover ricarlcolare la funzione di hash
        ...
        peso_ordine=ControllaEPrepara(Ricettario, Hk, Magazzino, pt, Lista_Ordini, ptr->quantita, ptr->ordine, 1);
...
void Ordine(RICETTARIO_P Ricettario, MAGAZZINO_P Magazzino, LO_Poynter Lista_Ordini){
    ...
    if(scanf("%s %d", r, &num)>0){
        ...
        if((Ricettario->vett[Rk].nome==NULL)||(strcmp(Ricettario->vett[Rk].nome, "tomb")==0)){
            ...
            peso_ordine=ControllaEPrepara(Ricettario, Rk, Magazzino, ptr, Lista_Ordini, num, r, 0);
            if(peso_ordine){
                printf("accettato\n");
...
int ControllaEPrepara(RICETTARIO_P Ricettario, int Rk, MAGAZZINO_P Magazzino, Ingrediente * primo_ingr_ricetta, LO_Poynter Lista_Ordini, int num, char * r, int flag){
    int n, peso_tot=0; //Mk
    Ingrediente * ptr=primo_ingr_ricetta;
    CHECK Check=NULL; //tmp=NULL;
    Lotto * pt =NULL, *temp=NULL;
    // int * v, i=0;
    // v=(int*)malloc(sizeof(int)*Ricettario->vett[Rk].n_ingredienti); //vettore dinamico dove salvo le quantità da deallocare
    ...
    } //uscito dal while sono sicuro di avere abbastanza ingredienti, e Check punta alle testate degli alberi che mi servono
    // while(Check!=NULL){
    //     pt=RB_Min(Check->albero->RBT);
    //     while((pt!=T_null)&&(v[i]>pt->quantita)){ //v[i] è la quantità totale per quell'ingrediente
    //         temp=pt;
    //         pt=RB_Successor(pt);
    //         v[i]-=temp->quantita;
    //         Check->albero->qt_tot-=temp->quantita;
    //         RB_Delete_M(Check->albero, temp);
    //     }
    //     if(pt!=T_null){
    //         Check->albero->qt_tot-=v[i];
    //         pt->quantita-=v[i];
    //     }
    //     i--;
    //     tmp=Check->prev;
    ...
    peso_tot=Ricettario->vett[Rk].peso*num;
    // free(v);
    return peso_tot;
}


CHECK Enqueue_CK (CHECK coda, RBTmagazzino RBT){
    CHECK punt;
